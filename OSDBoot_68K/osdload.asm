...
;@checktype:
		moveq		#0,d0	; Set pointer to first block MBR
		move.l		d0,_volstart
;		bsr			_read_secbuf
...
; PARTION ENTRY
; 00 00 00 00 0B 00 00 00 20 00 00 00 E0 EF 3A 00
;             |  |        |        \ Partition Size (offset 12)
;             |  |        \ LBA pointer in blocks (offset 8)
;             |  \ Don't care (offset 5)
;             \ Partition typo, 0x0B/0x0C FAT32 (offset 4)
...
;		move.b		#'l',RS232_base
		...
		swap		d0
		ror.w		#8,d0
		move.l		d0,_volstart	; Store volstart in BI
;		lea			_secbuf,a0
;		bsr			_read_secbuf
		bsr			cmd_read_sector	; read sector given value in D0
		bne.s		_error
		cmpi.b		#$55,$1fe(a0)   ; Check for magic 0xFFAA at the end of the block
		bne.s		_error
		cmpi.b		#$AA,$1ff(a0)
		beq.s		_testfat
;		bne.s		@error
...
; Dump D0 register to rs232, leaving d0 as it was when the routine was entered
dump_d0_debug:
		move.l      d1,_temp_d1
		move.l      a0,_temp_a0
		move.l      a1,_temp_a1
...
@loop:
		...
		lea         _hexrange,a1
		adda        d1,a1
		move.b      (a1),RS232_base   ; MOD: Add debug output lowest byte to RS232
		subq        #1,a0
		cmpa		#0,a0
		bne         @loop

		move.b      #$0a,RS232_base
		move.b      #$0d,RS232_base
...
; Dump A1 register to rs232, leaving a1 as it was when the routine was entered
dump_a1_debug:
		move.l		d0,_temp_d0
		move.l	    a1,d0
		bsr			dump_d0_debug
...
; Debug memory storage places
_hexrange           dc.b    "0123456789ABCDEF"
_temp_d0            ds.l    1   ;temp debug register for D0
